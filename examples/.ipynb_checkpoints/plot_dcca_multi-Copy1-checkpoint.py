...
if __name__ == '__main__':
    ...
    dcca = DCCA(
        embedding_path= args.embedPath,latent_dims=latent_dims, encoders=[Encoder_0, Encoder_1, Encoder_2], objective=objectives.GCCA, epochs= num_of_epochs
    )
    optimizer = optim.Adam(dcca.parameters(), lr=lr)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, 1)
    dcca = CCALightning(dcca, optimizer=optimizer, lr_scheduler=scheduler, embedding_path = args.embedPath)
    trainer = pl.Trainer(default_root_dir=args.log_path, max_epochs=num_of_epochs, enable_checkpointing=True, 
                         accelerator="gpu", devices=1, replace_sampler_ddp=False, profiler="simple", 
                         callbacks= [ModelCheckpoint(monitor='val loss', mode='min', dirpath=args.model_path, 
