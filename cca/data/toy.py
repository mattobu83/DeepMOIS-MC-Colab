...
class Split_MNIST_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print ( "Checj", x_a.shape, x_b.shape, x_c.shape)
        return (x_a, x_b,  x_c), label
    ...
    def to_numpy(self, indices=None):
        ...
        view_1 = np.zeros((len(indices), 266))
        view_2 = np.zeros((len(indices), 266))
        view_3 = np.zeros((len(indices), 266))
        labels = np.zeros(len(indices)).astype(int)
        for i, n in enumerate(indices):
            sample = self[n]
            view_1[i] = sample[0][0].numpy()
            view_2[i] = sample[0][1].numpy()
            view_3[i] = sample[0][2].numpy()
            labels[i] = sample[1].numpy().astype(int)
        return (view_1, view_2, view_3), labels
